...
Measurement follows each provider's quota mechanics: **sliding windows** (z.ai) are sampled every window-length — the API value at each boundary is exactly that window's trailing usage, so the tiling is exact; **fixed-reset windows** (Letta daily/4h, monthly pools) are measured at their *actual reset times* — the level just before each observed reset is the waste, avoiding the mid-window sampling error that would overcount. Windows the dashboard was DOWN for are reconstructed from metered usage when possible (tokens in the window vs capacity calibrated from observed windows) and marked "est." — no fabricated data without calibration. Daily rows show the average remaining-at-expiry across that day's windows, with an "hit 0% at least once" marker when any window ran dry.
